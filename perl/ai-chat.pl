...
our $opt_fix_level    = undef; # --fix-level：修正回复标题等级（undef=自动，0=不修，1=修正）
...
sub run {
    ...
    if ($is_stream) {
        # STDIN + -i 模式：先将缓冲的原始输入复制到 stdout
...
# 将响应打印到句柄（可带 ## role >> 标题）
# 标准输出时 fix-level 默认 0，可由 --fix-level 显式覆盖
sub print_response {
    my ($fh, $role, $content) = @_;
    $content = fix_heading_level($content) if ($opt_fix_level // 0);
    if ($opt_header) {
        print $fh "## $role >>\n\n";
    }
    print $fh $content, "\n";
...
# 原位追加响应到输入 .md 文件
# 写文件时 fix-level 默认 1，可由 --fix-level 显式覆盖
sub append_to_file {
    my ($file, $role, $content) = @_;
    $content = fix_heading_level($content) if ($opt_fix_level // 1);
...
选项（行为）:
  ...
  --stream           启用流式响应（SSE）；实时打印到 stdout；若同时指定 -i 则完成后也追加到文件
  --fix-level 0|1   修正回复标题等级（h1→h3, h2→h3, h3+各加一级）；-i 时默认 1，否则默认 0
...
与 C<-i> 合用时，流式输出结束后将完整响应追加到输入文件。
与 C<-j> 合用时，直接转发原始 SSE 行到标准输出。

=item B<--fix-level> I<0|1>

修正 AI 回复中的 Markdown 标题等级，防止二级标题干扰多轮对话的消息段解析。
值为 C<1> 时：h1（C<# >）升为 h3（C<### >），h2（C<## >）升为 h3，h3 及以上各加一级。
与 C<-i> 合用时默认 C<1>（写回文件时自动修正），否则默认 C<0>（标准输出不修正）。
