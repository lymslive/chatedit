...
our $opt_append       = 0;   # 回写输入文件（-a / --append）；STDIN 模式则复制输入到 STDOUT
our $opt_reformat     = undef; # --reformat 0|1：控制标题行 + 标题等级修正（undef=自动：写文件=1，stdout=0）
our $opt_env          = '';
our $opt_url          = '';
our $opt_key          = '';
our $opt_model        = '';
...
our $opt_json         = 0;   # -j/--json：直接输出原始 API 响应 JSON，忽略 -a
our $opt_postdir      = '';  # --postdir：将请求 JSON 保存到指定目录
our $opt_stream       = 0;   # --stream：启用流式响应（SSE），实时输出
...
sub run {
    ...
    # 检测并设置流式响应标记（--stream 或模板中 "stream":true 均可触发）
    ...
    # stdin + --append：在网络请求前先将原始输入复制到 stdout，让用户看到完整上下文
    ...
    if ($is_stream) {
        run_stream($request_json, $api_url, $api_key, $input_file);
    } else {
        run_non_stream($request_json, $api_url, $api_key, $input_file);
    }
}
...
# 非流式路径：等待完整响应后两阶段输出
sub run_non_stream {
    my ($request_json, $api_url, $api_key, $input_file) = @_;

    my $response_json = call_api($request_json, $api_url, $api_key);

    # --json：直接输出原始响应 JSON，忽略 -a
    if ($opt_json) {
        binmode STDOUT, ':raw';
        print $response_json;
        return;
    ...
    if ($opt_append && !defined $input_file) {
        # stdin + --append：stdout 充当"虚拟文件"，按文件模式输出（默认 reformat=1）
        print_response(\*STDOUT, $role, $content, 1);
    } else {
        # 阶段一：打印到 stdout（默认不 reformat）
        print_response(\*STDOUT, $role, $content, 0);

        # 阶段二：追加到实际文件（若 --append 且有输入文件）
        if ($opt_append && defined $input_file) {
...
# 流式路径：实时输出到 stdout，完成后可追加到文件
sub run_stream {
    my ($request_json, $api_url, $api_key, $input_file) = @_;

    if ($opt_json) {
        binmode STDOUT, ':raw';
        call_api_stream($request_json, $api_url, $api_key);
        return;
    }

    binmode STDOUT, ':utf8';
    local $| = 1;    # autoflush，确保 delta 实时输出

    # 确定 stdout 阶段的 reformat 策略
    # stdin + --append 时 stdout 充当"虚拟文件"，默认 reformat=1；否则默认 0
    my $stdout_reformat;
    if ($opt_append && !defined $input_file) {
    ...
    my ($role, $content) = call_api_stream($request_json, $api_url, $api_key);
...
# --json 模式直接转发原始 SSE 行；否则解析 delta 实时打印，返回 ($role, $accumulated_content)
sub call_api_stream {
    my ($json, $url, $key) = @_;
...
sub _extract_stream_delta {
    ...
    # Anthropic 原生流格式：content_block_delta / text_delta
    ...
    # Anthropic message_start：提取 role
...
# 修正 AI 回复中的 Markdown 标题等级（纯文本变换，由调用方决定是否调用）
# h1（# ）→ h3（### ），h2（## ）→ h3（### ），h3+ 各增加一级
# 列表上下文时返回 ($new_content, $reformed_lines_count)
...
sub append_to_file {
    ...
    my $do_fmt = defined $opt_reformat ? $opt_reformat : 1;  # 写文件默认 reformat=1
...
用法示例:
  # 发送请求并打印响应到 stdout（阶段一）
  perl/ai-chat.pl chat.md

  # 响应同时追加到文件（阶段二）；并在 stderr 打印摘要
