...
命令 `ai-chat.pl --stream --reformat 1` 一个示例输出如下：

```markdow
## assistant >>

在0 Claude 中，**Skill** 和 **Command** 确实有相似之处，但也存在重要区别：

##0 相同点

|0 特性 | 说明 |
|0------|------|
|0 调用方式 | 都可通过 `/` 触发（如 `/skill-name` 或 `/command`）|
|0 交互界面 | 都在输入框下方显示建议列表 |
|0 功能目的 | 都是为了扩展 Claude 的能力 |
